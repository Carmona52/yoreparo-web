...
const ESTADOS: { value: JobEstado; label: string; color: string; bg: string; icon: React.ReactNode }[] = [
    { value: "Pendiente",   label: "Pendiente",  color: "#E65100", bg: "rgba(245,124,0,0.12)",  icon: <HourglassEmptyIcon sx={{ fontSize: 18 }} /> },
    { value: "en proceso",  label: "En proceso", color: "#1565C0", bg: "rgba(21,101,192,0.12)", icon: <BuildCircleIcon   sx={{ fontSize: 18 }} /> },
    { value: "finalizado",  label: "Finalizado", color: "#2E7D32", bg: "rgba(46,125,50,0.12)",  icon: <CheckCircleIcon   sx={{ fontSize: 18 }} /> },
];

function getEstado(value: string) {
    return ESTADOS.find((e) => e.value.toLowerCase() === value?.toLowerCase()) ?? ESTADOS[0];
}
...
function SelectorEstado({ estadoActual, onCambiar, loading }: {
    estadoActual: string;
    onCambiar: (e: JobEstado) => void;
    loading: boolean;
}) {
    const idx = ESTADOS.findIndex((x) => x.value.toLowerCase() === estadoActual?.toLowerCase());

    return (
        <Card sx={{ borderRadius: 3, border: "1px solid rgba(0,0,0,0.07)", mb: 3 }}>
            <CardContent sx={{ p: 3 }}>
                                ...
                                <Box
                                    ...
                                    sx={{
                                        ...
                                        "&:hover": !isActive && !loading
                                            ? { borderColor: e.color, bgcolor: e.bg, color: e.color }
                                            : {},
                                    }}
                                >
                    ...
                    {ESTADOS.map((e, i) => {
                        ...
                        return (
                            <Box key={e.value} sx={{ display: "flex", alignItems: "center", flex: i < ESTADOS.length - 1 ? 1 : "none" }}>
                                <Box sx={{ width: 10, height: 10, borderRadius: "50%", bgcolor: done ? e.color : "rgba(0,0,0,0.12)", flexShrink: 0, transition: "background 0.3s" }} />
                                {i < ESTADOS.length - 1 && (
                                    <Box sx={{ flex: 1, height: 2, bgcolor: done && i < idx ? ESTADOS[i + 1].color : "rgba(0,0,0,0.08)", transition: "background 0.3s" }} />
                                )}
                            </Box>
                        );
                    })}
                ...
                <Box sx={{ display: "flex", justifyContent: "space-between", mt: 0.5 }}>
                    {ESTADOS.map((e) => (
                        <Typography key={e.value} variant="caption" sx={{ color: "text.secondary", fontSize: 10 }}>
                            {e.label}
                        </Typography>
                    ))}
                </Box>
...
export default function ServicioDetallePage() {
        ...
        try {
            ...
            if (data.worker_id) {
                const { data: perfil } = await supabase
                    .from("profiles").select("*").eq("id", data.worker_id).single();
    ...
    useEffect(() => { void cargarServicio(); }, [id]);
    ...
    async function handleCambiarEstado(nuevoEstado: JobEstado) {
        ...
        try {
            ...
            setSnack({
                open: true,
                msg: servicio.worker_id
                    ? `Estado cambiado a "${nuevoEstado}" · Técnico notificado`
    ...
    return (
        // Sin maxWidth fijo — ocupa todo el ancho del layout del dashboard
        // El padding horizontal lo maneja el layout padre
        <Box>
            ...
            <Card sx={{ borderRadius: 3, border: "1px solid rgba(0,0,0,0.07)", mb: 3 }}>
                <CardContent sx={{ p: 0, "&:last-child": { pb: 0 } }}>
                    {servicio.image_url ? (
                        <Box component="img" src={servicio.image_url} alt={servicio.title}
                             sx={{ width: "100%", height: 220, objectFit: "cover", borderRadius: "12px 12px 0 0" }} />
                    ) : (
                        <Box sx={{ width: "100%", height: 100, bgcolor: "rgba(255,214,0,0.08)", borderRadius: "12px 12px 0 0", display: "flex", alignItems: "center", justifyContent: "center" }}>
                    ...
                    <Box sx={{ p: 3 }}>
                        ...
                        <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", mb: 2, gap: 1 }}>
                            ...
                            <Chip
                                ...
                                size="small"
                                sx={{ bgcolor: estado.bg, color: estado.color, fontWeight: 700, fontSize: 12, height: 28, flexShrink: 0 }}
                            />
                        </Box>
                            ...
                            <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                                ...
                                <Typography variant="body2" sx={{ color: "text.secondary" }}>
                                    ...
                                    <Typography component="span" variant="body2" sx={{ fontWeight: 700, color: "text.primary" }}>
                                        {servicio.price ? `$${servicio.price.toLocaleString("es-MX")}` : "—"}
                                    </Typography>
                                </Typography>
                            </Box>
                            <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
                        ...
                        {servicio.cotizacion_id && (
                            <>
                                <Divider sx={{ my: 2.5 }} />
                                <Button fullWidth variant="outlined" size="medium" startIcon={<ReceiptIcon />}
                                        onClick={() => router.push(`/dashboard/cotizaciones/${servicio.cotizacion_id}`)}
                                        sx={{ borderRadius: 2, fontWeight: 700, borderColor: "#FFD600", color: "#B8860B", "&:hover": { bgcolor: "rgba(255,214,0,0.08)", borderColor: "#FFD600" } }}>
                    ...
                    {tecnico ? (
                        <>
                            <Box sx={{ display: "flex", alignItems: "center", gap: 2 }}>
                                <Avatar sx={{ width: 52, height: 52, bgcolor: "#FFD600", color: "#1A1A2E", fontWeight: 800, fontSize: 20 }}>
                                    {tecnico.name?.charAt(0).toUpperCase() ?? "T"}
                                </Avatar>
                                <Box sx={{ flex: 1, minWidth: 0 }}>
                                    <Typography sx={{ fontWeight: 700, fontSize: 15 }}>{tecnico.name}</Typography>
                                    <Chip label={tecnico.role} size="small"
                                ...
                                <Tooltip title="Enviar notificación al técnico">
                                    <IconButton size="small"
                                                onClick={() =>
                                                    serviciosService.notificarTecnico(servicio.worker_id!, servicio.title, servicio.status as JobEstado)
                                                        .then(() => setSnack({ open: true, msg: "Notificación enviada al técnico", severity: "info" }))
                                                }
            ...
            >
                <Alert severity={snack.severity} onClose={() => setSnack((s) => ({ ...s, open: false }))} sx={{ borderRadius: 3, fontWeight: 600 }}>
                    {snack.msg}
                </Alert>
            </Snackbar>
        </Box>
